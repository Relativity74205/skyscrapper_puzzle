...
class SkyscraperGrid:
    ...
    def get_array(self, dim, i, reverse):
        ...
        arr = [ele[0] if len(ele) == 1 else ele for ele in arr]
    ...
    def violates_constraints(self, p, num, initial=False):
        check_points = [point for point in all_neighbour_points[tuple(p)] if len(self.grid[point[0]][point[1]]) == 1]
        ...
        for clue_index in clue_indizes:
            ...
            if clue > 0:
                arr = self.get_array(*self.get_column(clue_index))

                if self.violates_constraints_array(arr, clue, initial=initial):
    ...
    def violates_constraints_array(self, arr, clue, initial=False):
        visible_buildings = self.get_visible_buildings(arr)
        if visible_buildings != clue and all([True if type(ele) == int else False for ele in arr]):
            return True

        if initial:
            if visible_buildings > clue:
                return True
    ...
    def get_visible_buildings(self, arr):
        clues = 0
        for i in range(self.grid_dimension):
...
if __name__ == "__main__":
    ...
    print(solve_puzzle(clues_6_array[1]))
